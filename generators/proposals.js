const generatePTBCommand = ({ packageId, dashboardId, numProposals, includeDescription = true }) => {
  ...
  for (let i = 1; i <= numProposals; i++) {
    ...
    // Description is now Option<String> - can be Some(value) or None
    // In Sui PTB, when passing Option<String>:
    // - For Option::some(value): pass the string value directly (Sui auto-wraps)
    // - For Option::none: use empty vector [] or std::option::none<String>()
    const descriptionArg = includeDescription 
      ? `'"Proposal description ${timestampId}"'` 
      : '[]'; // Empty vector represents Option::none

    // Add proposal creation command (Windows-style)
    // Note: description is now Option<String> in the contract
    // Format: title (String), description (Option<String>), expiration (u64), blobs_id (String)
    command += `
  --move-call ${packageId}::proposal::create \`
  '"${title}"' ${descriptionArg} ${timestamp} '"${blodID}"' \`
  --assign proposal_id \`
...
const inputs = {
  ...
  includeDescription: true, // true để tạo proposal có description, false để tạo proposal không có description (Option::none)
};
